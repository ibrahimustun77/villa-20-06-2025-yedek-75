...
export const updateVilla = async (id: string, villa: { 
  ...
  try {
    console.log('Updating villa with complete data:', id, villa);
    console.log('Villa feature_groups to save:', villa.feature_groups);
    console.log('Villa dynamic_rooms to save:', villa.dynamic_rooms);
    
    // Convert types to match database schema and ensure all icon fields are included
    ...
    console.log('Villa update data prepared for database:', villaData);
    ...
    console.log('Villa updated successfully:', data);

    // Sync villa room counts using the new service with automatic trigger sync
    if (villa.dynamic_rooms !== undefined) {
      console.log('Syncing villa room counts:', villa.dynamic_rooms);
      await syncVillaRoomCounts(id, villa.dynamic_rooms || []);
    }
    
    toast({
      title: "Başarılı",
      description: 'Villa başarıyla güncellendi'
    });
    ...
    toast({
      title: "Hata",
      description: `Villa güncellenirken hata oluştu: ${error.message}`,
      variant: "destructive"
    });
    return null;
  }
